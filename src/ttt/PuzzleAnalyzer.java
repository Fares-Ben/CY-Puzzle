...
public class PuzzleAnalyzer {
    ...
    private List<List<Point>> splitEdges(List<Point> contour) {
    ...
    for (Point p : contour) {
        ...
        if (allRight  && allBottom && noLeft  && noTop){
            coins.add(p);
            finalCoins[0] = x;
            finalCoins[1] = y;
        
        }    
        else if (allLeft   && allBottom && noRight && noTop){
            coins.add(p);
            finalCoins[2] = x;
            finalCoins[3] = y;
        }    
        else if (allLeft   && allTop    && noRight && noBottom){
            coins.add(p);
            finalCoins[4] = x;
            finalCoins[5] = y;
        }  
        else if (allRight  && allTop    && noLeft  && noBottom){
            coins.add(p);
            finalCoins[6] = x;
            finalCoins[7] = y;
        }  
    ...
    private EdgeResult analyserBord(List<Point> edge, Side side, BufferedImage img) {
        ...
        if (edge.isEmpty()) {
            return new EdgeResult(2, lengths, new int[3],0,new int[3]);
        }
        for (Point p : edge){
        ...
        if (middle.size() > 1) {
            ...
            // 3) variables pour les extrêmes parallèles
            int minPar = Integer.MAX_VALUE;
            int maxPar = Integer.MIN_VALUE;
            Point minParPoint = null;
            Point maxParPoint = null;
            ...
            for (Point p : middle) {
                ...
                // mise à jour des extêmes parallèles
                int parCoord = (axe == 1) ? p.x : p.y;  // coordonnée parallèle au bord
                if (parCoord < minPar) {
                    minPar      = parCoord;
                    minParPoint = p;
                }
            ...
            // 5) second parcours : comptage des points sur chaque extrême
            ...
            //System.out.println("minpar : "+ countMinPar + " perp : " + countPerp + " maxpar : " + countMaxPar);
        ...
        */
        int[] maxPixel = new int[3];
        maxPixel[0] = countMinPar;
        maxPixel[1] = countPerp;
        maxPixel[2] = countMaxPar;
        // -------- Calcul des moyennes RGB --------
        for (int k = 0; k < 3; k++) {
            rgbMeans[k] = (lengths[k] == 0) ? 0 : (rgbSums[k] / lengths[k]);
        }

        return new EdgeResult(type, lengths, rgbMeans, profondeur,maxPixel);
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        File    file1 = new File("/home/cytech/Desktop/cat/25x16/img_6_12.png");
        BufferedImage img1 = ImageIO.read(file1);

        File    file2 = new File("/home/cytech/Desktop/cat/25x16/img_4_14.png");
